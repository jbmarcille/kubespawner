...
class KubeSpawner(Spawner):
    ...
    singleuser_notebook_dir = Unicode(
        '/data/jupyterhub/notebooks',
        config=True,
        help="""
        The directory where the notebooks will be stored inside the singleuser POD.
    ...
    def options_from_form(self, formdata):
        image = formdata.get('dockerimage', ['registry.airmes.sparkindata.com/airmes/jupyter-singleuser:1.1.1'])[0]
        if image: 
    ...
    def start(self):
        ...
        if self.hub_service_name:
            ...
            self.accessible_hub_api_url = urlunparse((scheme, netloc, path, params, query, fragment))
            self.log.info('Hub service url: {huburl}'.format(huburl=self.accessible_hub_api_url))
